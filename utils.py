...
def retImage(images,size):
    return merge(inverse_transform(images),size)

def imread(path):
    return imageio.imread(path,format='.png').astype(np.float)
...
def center_crop(x, crop_h, crop_w=None, resize_w=64):
    ...
    return Image.fromarray(x[j:j+crop_h, i:i+crop_w]).resize(size=([resize_w, resize_w]))
